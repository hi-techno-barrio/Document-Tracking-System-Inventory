...
if(isset($_GET['rs_id']))
	...
	if($opr=="del")
{
	$del_sql=mysqli_query($conn,"DELETE FROM tenuretbl WHERE tenuretbl_id=$id");
	if($del_sql)
...
<table width="755">
	<tr>
        ...
        <td>
        	<a href="?tag=tenure_entry"><input type="button" title="Add new Documents" name="butAdd" value="Add New" id="button-search" /></a>
        </td>
        <td>
...
<form method="post">
	...
	if($key !="")
		$sql_sel=mysqli_query($conn,"SElECT * FROM tenuretbl WHERE First_Name like '%$key%' or Last_Name like '%$key%'");
	    //$sql_sel="SElECT * FROM stu_tbl WHERE f_name  like '%$key%' or l_name like '%$key%'";
	else
		 $sql_sel=mysqli_query($conn,"SELECT * FROM tenuretbl");
    ...
    ?>	
      <tr bgcolor="<?php echo $color?>">
            ...
            <td><a href="?tag=tenure_entry&opr=upd&rs_id=<?php echo $row['tenuretbl_id'];?>" title="Update"><img src="picture/update.png" /></a></td>
            <td><a href="?tag=view_tenure&opr=del&rs_id=<?php echo $row['tenuretbl_id'];?>" title="Delete"><img src="picture/delete.png" /></a></td>
